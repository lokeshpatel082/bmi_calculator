import 'package:flutter/material.dart';

const kLabelTextStyle = TextStyle(
  fontSize: 13.0,
  color: Color(0xFFB8B9C6),
);

const kBottomContainerColor = Color(0xFFFF0067);
const kActiveCardColor = Color(0xFF222747);
const kInactiveCardColor = Color(0xFF14193B);
const kNumberTextStyle =
    TextStyle(fontWeight: FontWeight.w900, color: Colors.white, fontSize: 40.0);
const kResultTitleText = TextStyle(color: Color(0xFF21CE72), fontSize: 19.0);
const kBMI = TextStyle(fontWeight: FontWeight.bold, fontSize: 70);
const kBodyTextStyle = TextStyle(
  fontSize: 19.0,
);
